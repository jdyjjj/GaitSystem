...
def img2pickle(input_path: Path, output_path: Path, img_size: int = 64, verbose: bool = False, dataset='CASIAB'):
    """Reads a group of images and saves the data in pickle format.
    ...
    for img_path in input_path.rglob('*.png'):
        *_, sid, seq, view, _ = img_path.as_posix().split('/')
        ...
        total_files += 1
    # print(img_groups,33)
    # img_groups = img_groups.items
    # print(img_groups.items,55)
    print(sinfos)
    for sinfo in sinfos:
        img_paths = img_groups[(sinfo[0], sinfo[1], sinfo[2])]
        # print(type(img_groups))
        # print(img_groups.items())
        to_pickle = []
        # print(img_paths,1)
        for img_file in sorted(img_paths):
...
def pretreat(input_path: Path, output_path: Path, img_size: int = 64, workers: int = 4, verbose: bool = False, dataset: str = 'CASIAB') -> None:
    ...
    with mp.Pool(workers) as pool:
        ...
        for a in pool.imap_unordered(partial(imgs2pickle, output_path=output_path, img_size=img_size, verbose=verbose, dataset=dataset), img_groups.items()):
            progress.update(1)
            result.append(a)
            # print(a)
    logging.info('Done')
